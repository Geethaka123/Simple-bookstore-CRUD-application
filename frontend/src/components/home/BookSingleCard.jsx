...
const BookSingleCard = ({ book }) => {
  const [showModel, setShowModel] = useState(false)
    ...
    <div
      key={book._id}
      className='border-2 border-gray-500 rounded-lg px-4 py-2 m-4 relative hover:shadow-xl bg-[#cffafe]'
    >
      <h2 className='absolute top-1 right-2 px-4 py-1 rounded-lg bg-[#0e7490]'>
        {book.publishYear}
      </h2>
      <h4 className='my-2 text-gray-500'>{book._id}</h4>
      <div className='flex justify-start items-center gap-x-2'>
        <PiBookOpenTextLight className='text-[#0e7490] text-2xl' />
        <h2 className='my-1'>{book.title}</h2>
      </div>
      <div className='flex justify-start items-center gap-x-2'>
        <BiUserCircle className='text-[#0e7490] text-2xl' />
